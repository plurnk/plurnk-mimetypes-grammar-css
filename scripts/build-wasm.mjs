#!/usr/bin/env node
// Reproducible WASM build for tree-sitter-css.
...
const work = await mkdtemp(path.join(tmpdir(), "grammar-css-build-"));
await run("git", ["clone", "--no-checkout", "https://github.com/tree-sitter/tree-sitter-css.git", "src"], { cwd: work });
...
// Locate produced wasm and copy as css.wasm.
const fs = await import("node:fs/promises");
const built = (await fs.readdir(buildCwd)).find((f) => f.endsWith(".wasm"));
if (!built) throw new Error("no .wasm produced");
await copyFile(path.join(buildCwd, built), path.join(repoRoot, "css.wasm"));
const bytes = (await readFile(path.join(repoRoot, "css.wasm"))).length;
console.log(`css.wasm: ${bytes} bytes (built from ${pin})`);
