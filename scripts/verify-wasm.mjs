#!/usr/bin/env node
// CI byte-identical reproducibility check for css.wasm.
...
const committed = await readFile(path.join(repoRoot, "css.wasm"));
const committedHash = createHash("sha256").update(committed).digest("hex");
console.log(`committed css.wasm sha256: ${committedHash}`);

const work = await mkdtemp(path.join(tmpdir(), "grammar-css-verify-"));
...
console.log(`rebuilt css.wasm sha256: ${rebuiltHash}`);
